# ========== 使用说明 ==========
# make build PLATFORM=linux/arm64    # 构建镜像，默认 linux/amd64
# make build-node24 PLATFORM=linux/arm64  # 构建 node24 镜像
# make check PLATFORM=linux/arm64     # 运行最新版本镜像测试
# make check-node24 PLATFORM=linux/arm64  # 运行 node24 镜像测试
# make push PLATFORM=linux/arm64      # 推送所有版本镜像
# make push-node24 PLATFORM=linux/arm64   # 推送 node24 镜像
...
build:
	@for nodeVersion in $(BUILD_NODE_IMAGE); do \
		...
		echo "\nBuilding Docker image for $$imageName with platform $(PLATFORM)..."; \
		docker build --platform $(PLATFORM) --build-arg NODE_VERSION=$$nodeVersion -t $$imageName .; \
    done

build-node24:
	@imageName=ghcr.io/$(DOCKER_USERNAME)/$(PROJECT_NAME):$(PROJECT_VERSION)-node24; \
	echo "\nBuilding Docker image for $$imageName with platform $(PLATFORM)..."; \
	docker build --platform $(PLATFORM) --build-arg NODE_VERSION=24 -t $$imageName .
